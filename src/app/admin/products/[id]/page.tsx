...
export default function AdminEditProductPage({ params }: EditProductPageProps) {
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        ...
        const res = await adminGet(`/api/products/${id}`)
        ...
        setFormData({
          ...
          commands: data.commands?.length > 0 ? data.commands : [''],
          ...
          saleStart: formatDateTimeLocal(data.saleStart),
          saleEnd: formatDateTimeLocal(data.saleEnd),
        })
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminPut(`/api/products/${id}`, submitData)
      ...
      toastError(err.message || 'เกิดข้อผิดพลาดในการบันทึกการแก้ไขสินค้า')
    }
  }
  ...
  const addCommand = () => setFormData({ ...formData, commands: [...formData.commands, ''] })
  ...
  const removeCommand = (index: number) => {
    setFormData({ ...formData, commands: formData.commands.filter((_, i) => i !== index) })
  }
  ...
  return (
    <div className="admin-form-page">
      ...
      <h1 className="admin-form-page-title">แก้ไขสินค้า: {formData.name}</h1>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className="form-row" style={{ marginTop: '1rem' }}>
            <FormField label="ชื่อสินค้า" required>
              <input type="text" className="input" value={formData.name}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder="เช่น ดาบเนเธอไรต์สุดเท่" />
            </FormField>
            <FormField label="หมวดหมู่" required>
              <AdminDropdown value={formData.categoryId} options={categoryOptions}
                onChange={(val) => setFormData({ ...formData, categoryId: val })} placeholder="เลือกหมวดหมู่" />
            </FormField>
          </div>
          ...
          <div className="form-row" style={{ marginTop: '0.875rem' }}>
            <FormField label="ราคา (บาท)" required>
              <input type="number" className="input" value={formData.price}
                onChange={(e) => setFormData({ ...formData, price: e.target.value })} required min="0" step="any" />
            </FormField>
            <FormField label="คำอธิบาย">
              <textarea className="input" rows={1} value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="อธิบายสินค้าสั้นๆ..."
                style={{ resize: 'vertical', minHeight: '42px' }} />
            </FormField>
          </div>

          <hr className="section-divider" />
          <AdminToggle title="เปิดขายสินค้านี้ทันที" description="สินค้าจะแสดงในหน้าร้านค้า"
        ...
        <SectionCard title="คำสั่ง RCON" icon={<TerminalIcon size={15} />}>
          <FormField label="รายการคำสั่ง" hint={<>ใช้ <span className="code-inline">{'{player}'}</span> สำหรับชื่อผู้ใช้ และ <span className="code-inline">{'{customInput}'}</span> สำหรับข้อมูลกำหนดเอง</>}>
            <div className="cmd-list">
              {formData.commands.map((cmd, index) => (
                <div key={index} className="cmd-row">
                  <span className="cmd-prefix">/</span>
                  <input type="text" className="input cmd-input" value={cmd}
                    onChange={(e) => updateCommand(index, e.target.value)} placeholder="give {player} diamond 64" />
                  {formData.commands.length > 1 && (
                    <button type="button" className="btn-remove" onClick={() => removeCommand(index)}><CloseIcon size={14} /></button>
                  )}
                </div>
              ))}
            </div>
          ...
          <hr className="section-divider" />
          <AdminToggle title="ต้องการข้อมูลเพิ่มจากผู้เล่น" description="เปิดใช้เมื่อต้องให้ผู้เล่นกรอกข้อมูลก่อนซื้อ"
            checked={formData.requiresInput} onChange={(val) => setFormData({ ...formData, requiresInput: val })} />

          {formData.requiresInput && (
            <div className="form-row" style={{ marginTop: '0.75rem' }}>
              <FormField label="หัวข้อ (Label)">
                <input type="text" className="input" value={formData.inputLabel}
                  onChange={(e) => setFormData({ ...formData, inputLabel: e.target.value })} placeholder="เช่น สีที่ต้องการ" />
              </FormField>
              <FormField label="คำใบ้ (Placeholder)">
                <input type="text" className="input" value={formData.inputPlaceholder}
                  onChange={(e) => setFormData({ ...formData, inputPlaceholder: e.target.value })} placeholder="เช่น &bYourName" />
              </FormField>
            </div>
          )}
        </SectionCard>
        ...
        <SectionCard title="ราคาพิเศษ & โปรโมชัน" icon={<TagSaleIcon size={15} />}>
          <AdminToggle title="เปิดใช้ราคาพิเศษ" description="ตั้งค่าส่วนลดสำหรับสินค้าชิ้นนี้"
            checked={formData.saleActive} onChange={(val) => setFormData({ ...formData, saleActive: val })} />
              ...
              <div className="form-row" style={{ marginTop: '0.75rem' }}>
                ...
                <FormField label={formData.discountType === 'PERCENTAGE' ? 'ลดกี่เปอร์เซ็นต์ (%)' : 'ลดกี่บาท (฿)'}>
                  <input type="number" className="input" value={formData.discountValue}
                    onChange={(e) => setFormData({ ...formData, discountValue: e.target.value })} min="0" step="any" />
                </FormField>
              </div>
              <div style={{ marginTop: '1rem' }}>
                <CustomDateTimePicker startDate={formData.saleStart} endDate={formData.saleEnd}
                  onChange={(start, end) => setFormData(prev => ({ ...prev, saleStart: start, saleEnd: end }))} label="ช่วงเวลาโปรโมชัน" />
              </div>
            </>
          )}
        </SectionCard>
      ...
      <ConfirmModal
        ...
        content={`คุณแน่ใจหรือไม่ว่าต้องการลบสินค้า "${formData.name}" ถาวร? การกระทำนี้ไม่สามารถกู้ข้อมูลกลับคืนได้`}
